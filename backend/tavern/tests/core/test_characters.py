...
class TestProficiencyBonus:
    def test_level_1_returns_2(self) -> None:
        assert proficiency_bonus(1) == 2

    def test_level_4_returns_2(self) -> None:
        assert proficiency_bonus(4) == 2

    def test_level_5_returns_3(self) -> None:
        assert proficiency_bonus(5) == 3

    def test_level_8_returns_3(self) -> None:
        assert proficiency_bonus(8) == 3

    def test_level_9_returns_4(self) -> None:
        assert proficiency_bonus(9) == 4

    def test_level_12_returns_4(self) -> None:
        assert proficiency_bonus(12) == 4

    def test_level_13_returns_5(self) -> None:
        assert proficiency_bonus(13) == 5

    def test_level_16_returns_5(self) -> None:
        assert proficiency_bonus(16) == 5

    def test_level_17_returns_6(self) -> None:
        assert proficiency_bonus(17) == 6

    def test_level_20_returns_6(self) -> None:
    ...
    def test_level_21_raises(self) -> None:
        with pytest.raises(ValueError):
            proficiency_bonus(21)


class TestMaxHpAtLevel1:
    def test_barbarian_con_plus2_returns_14(self) -> None:
        assert max_hp_at_level_1("Barbarian", 2) == 14

    def test_wizard_con_minus1_returns_5(self) -> None:
        assert max_hp_at_level_1("Wizard", -1) == 5

    def test_fighter_no_con_returns_10(self) -> None:
        assert max_hp_at_level_1("Fighter", 0) == 10

    def test_minimum_hp_is_1(self) -> None:
        # Sorcerer (d6) with Con -5 would be 1, not 0 or negative
        assert max_hp_at_level_1("Sorcerer", -5) == 1

    def test_unknown_class_raises(self) -> None:
        with pytest.raises(ValueError, match="Unknown class"):
            max_hp_at_level_1("Peasant", 0)


class TestHpGainedOnLevelUp:
    def test_barbarian_fixed_value_with_bonus(self) -> None:
        # Barbarian fixed = 7, Con +2 → 9
        assert hp_gained_on_level_up("Barbarian", 2) == 9

    def test_wizard_fixed_value_with_penalty(self) -> None:
        # Wizard fixed = 4, Con -1 → 3
        assert hp_gained_on_level_up("Wizard", -1) == 3

    def test_minimum_hp_gained_is_1(self) -> None:
        # Wizard (4) with Con -5 would be -1; should be clamped to 1
        assert hp_gained_on_level_up("Wizard", -5) == 1

    def test_use_fixed_true_by_default(self) -> None:
        result = hp_gained_on_level_up("Cleric", 0)
        assert result == 5  # fixed value for d8 class

    def test_unknown_class_raises(self) -> None:
        with pytest.raises(ValueError, match="Unknown class"):
            hp_gained_on_level_up("Peasant", 0)


class TestSpellSlots:
    # Full casters
    def test_wizard_level_1(self) -> None:
        assert spell_slots("Wizard", 1) == {1: 2}

    def test_wizard_level_3(self) -> None:
        assert spell_slots("Wizard", 3) == {1: 4, 2: 2}

    def test_wizard_level_20(self) -> None:
        expected = {1: 4, 2: 3, 3: 3, 4: 3, 5: 3, 6: 2, 7: 2, 8: 1, 9: 1}
        assert spell_slots("Wizard", 20) == expected

    def test_cleric_level_1(self) -> None:
        assert spell_slots("Cleric", 1) == {1: 2}

    def test_bard_level_5(self) -> None:
        assert spell_slots("Bard", 5) == {1: 4, 2: 3, 3: 2}

    # Half casters (SRD 5.2.1: slots from level 1)
    def test_paladin_level_1(self) -> None:
        assert spell_slots("Paladin", 1) == {1: 2}

    def test_paladin_level_2(self) -> None:
        assert spell_slots("Paladin", 2) == {1: 2}

    def test_ranger_level_1(self) -> None:
        assert spell_slots("Ranger", 1) == {1: 2}

    def test_paladin_level_5(self) -> None:
        assert spell_slots("Paladin", 5) == {1: 4, 2: 2}

    # Warlock pact magic
    def test_warlock_level_1(self) -> None:
        assert spell_slots("Warlock", 1) == {1: 1}

    def test_warlock_level_2(self) -> None:
        assert spell_slots("Warlock", 2) == {1: 2}

    def test_warlock_level_5(self) -> None:
    ...
    def test_fighter_level_5_returns_empty(self) -> None:
        assert spell_slots("Fighter", 5) == {}

    def test_barbarian_level_10_returns_empty(self) -> None:
        assert spell_slots("Barbarian", 10) == {}

    def test_monk_level_3_returns_empty(self) -> None:
        assert spell_slots("Monk", 3) == {}

    # Edge cases
    def test_unknown_class_raises(self) -> None:
        with pytest.raises(ValueError, match="Unknown class"):
            spell_slots("Peasant", 1)

    def test_level_0_raises(self) -> None:
        with pytest.raises(ValueError):
            spell_slots("Wizard", 0)

    def test_level_21_raises(self) -> None:
        with pytest.raises(ValueError):
            spell_slots("Wizard", 21)

    def test_returns_copy_not_reference(self) -> None:
        result = spell_slots("Wizard", 1)
        result[1] = 99
        assert spell_slots("Wizard", 1) == {1: 2}


class TestMulticlassSpellSlots:
    def test_wizard5_fighter5(self) -> None:
        # Wizard is full caster (level 5), Fighter non-caster (0)
        # Combined level = 5, look up _FULL_CASTER_SLOTS[4]
        result = multiclass_spell_slots({"Wizard": 5, "Fighter": 5})
        assert result == {1: 4, 2: 3, 3: 2}

    def test_wizard5_paladin4(self) -> None:
        # Wizard 5 (full=5) + Paladin 4 (half=ceil(4/2)=2) = combined 7
        result = multiclass_spell_slots({"Wizard": 5, "Paladin": 4})
        # Level 7 = {1: 4, 2: 3, 3: 3, 4: 1}
        assert result == {1: 4, 2: 3, 3: 3, 4: 1}

    def test_paladin2_ranger2(self) -> None:
        # Paladin 2 (ceil(2/2)=1) + Ranger 2 (ceil(2/2)=1) = combined 2
        result = multiclass_spell_slots({"Paladin": 2, "Ranger": 2})
        # Level 2 = {1: 3}
        assert result == {1: 3}

    def test_warlock_only(self) -> None:
        # Warlock is separate — pact magic only
        result = multiclass_spell_slots({"Warlock": 3})
        assert result == {2: 2}

    def test_wizard3_warlock3(self) -> None:
        # Wizard 3 combined_level=3; Warlock level 3 pact magic = {2: 2}
        # Table level 3 = {1: 4, 2: 2}; warlock adds {2: 2} → {2: 4}
        result = multiclass_spell_slots({"Wizard": 3, "Warlock": 3})
        assert result[1] == 4
        assert result[2] == 4  # 2 from table + 2 from warlock pact magic

    def test_fighter_only_returns_empty(self) -> None:
        result = multiclass_spell_slots({"Fighter": 10})
        assert result == {}

    def test_half_caster_rounding_up(self) -> None:
        # Paladin 3: ceil(3/2) = 2
        # Paladin 1: ceil(1/2) = 1
        result_3 = multiclass_spell_slots({"Paladin": 3})
        result_1 = multiclass_spell_slots({"Paladin": 1})
    ...
    def test_invalid_class_raises(self) -> None:
        with pytest.raises(ValueError, match="Unknown class"):
            multiclass_spell_slots({"Peasant": 5})

    def test_invalid_level_raises(self) -> None:
        with pytest.raises(ValueError):
            multiclass_spell_slots({"Wizard": 21})


class TestLevelForXp:
    def test_0_xp_returns_level_1(self) -> None:
        assert level_for_xp(0) == 1

    def test_299_xp_returns_level_1(self) -> None:
        assert level_for_xp(299) == 1

    def test_300_xp_returns_level_2(self) -> None:
        assert level_for_xp(300) == 2

    def test_355000_xp_returns_level_20(self) -> None:
        assert level_for_xp(355000) == 20

    def test_above_max_xp_returns_level_20(self) -> None:
        assert level_for_xp(999999) == 20

    def test_xp_thresholds_all_correct(self) -> None:
        expected_pairs = [
            (0, 1),
            (300, 2),
            (900, 3),
    ...
    def test_one_below_threshold_stays_lower(self) -> None:
        # Just below level 3 threshold
        assert level_for_xp(XP_THRESHOLDS[2] - 1) == 2
...
class TestCanMulticlass:
    def test_barbarian_to_fighter_with_str_13(self) -> None:
        # Barbarian needs STR≥13, Fighter needs STR≥13
        assert can_multiclass({"Barbarian": 3}, "Fighter", {"STR": 13, "DEX": 10}) is True

    def test_barbarian_to_wizard_low_int(self) -> None:
        # Wizard needs INT≥13
        assert can_multiclass({"Barbarian": 3}, "Wizard", {"STR": 15, "INT": 10}) is False

    def test_wizard_to_fighter_int_13_str_13(self) -> None:
        assert can_multiclass({"Wizard": 5}, "Fighter", {"INT": 15, "STR": 13}) is True

    def test_monk_needs_dex_and_wis(self) -> None:
        # Monk requires DEX≥13 AND WIS≥13
        assert can_multiclass({}, "Monk", {"DEX": 13, "WIS": 13}) is True
        assert can_multiclass({}, "Monk", {"DEX": 13, "WIS": 12}) is False
        assert can_multiclass({}, "Monk", {"DEX": 12, "WIS": 13}) is False

    def test_paladin_needs_str_and_cha(self) -> None:
        assert can_multiclass({}, "Paladin", {"STR": 13, "CHA": 13}) is True
        assert can_multiclass({}, "Paladin", {"STR": 13, "CHA": 12}) is False

    def test_ranger_needs_dex_and_wis(self) -> None:
        assert can_multiclass({}, "Ranger", {"DEX": 13, "WIS": 13}) is True
        assert can_multiclass({}, "Ranger", {"DEX": 14, "WIS": 12}) is False

    def test_current_class_also_checked(self) -> None:
        # Barbarian needs STR≥13; if character has STR 12, they can't even
        # have legally been a Barbarian but we still validate
        assert can_multiclass({"Barbarian": 3}, "Rogue", {"STR": 12, "DEX": 13}) is False

    def test_single_class_character(self) -> None:
        assert can_multiclass({}, "Rogue", {"DEX": 13}) is True

    def test_unknown_current_class_raises(self) -> None:
        with pytest.raises(ValueError, match="Unknown class"):
            can_multiclass({"Peasant": 1}, "Fighter", {"STR": 15})

    def test_unknown_new_class_raises(self) -> None:
        with pytest.raises(ValueError, match="Unknown class"):
            can_multiclass({"Fighter": 1}, "Peasant", {"STR": 15})

    def test_all_primary_abilities_covered(self) -> None:
